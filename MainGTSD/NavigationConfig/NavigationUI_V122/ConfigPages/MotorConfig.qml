...
Item {
    ...
    property var chineseNameMap: {
        "Imax":"最大电流","Irat":"额定电流","Sct":"额定转速","Srat":"过速百分比","Nos":"最大转速","Rm":"电阻","Ldm":"L轴电感",
    ...
    function aHeadExecuteCommand(){
        ...
        if(mg_servoCmd.axisIndex!==curSelectAxis){
            ...
            for(var i=0;i<m_motorPrmModel.count;i++){
                ...
                console.log(qsTr("contorlKey:%1 \tvalue:%2").arg(controlName).arg(retValue));
                m_motorPrmModel.setProperty(i,"value",qsTr("%1").arg(retValue));
            }
