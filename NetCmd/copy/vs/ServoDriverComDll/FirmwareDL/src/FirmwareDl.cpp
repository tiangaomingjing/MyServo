...
int16 CFirmwareDL::CheckRemoteUpdataState(int16 com_type,int32 delaytimes /*= DELAY_TIMES*/)
{
	...
	while (delaytimes--)
	{
		iRet = g_AbsCom->GTSD_Com_Firmware_handler(com_type, GTSD_COM_MODE_READ, comAddr, &data, comNum);////查询完成标志
		if (iRet == 0)
		{
...
int16  CFirmwareDL::SetRemoteUpdataReadRequest(int16 com_type,Uint32 flash_addr, Uint16 iLength)
{
	...
	iRet = g_AbsCom->GTSD_Com_Firmware_handler(com_type, GTSD_COM_MODE_WRITE, comAddr, (int16*)(data), comNum); //写入读取命令
	if(iRet!=0)
	{
		return -1;
	}
...
int16  CFirmwareDL::ProtectOff(int16 com_type)
{
	...
	iRet = g_AbsCom->GTSD_Com_Firmware_handler(com_type, GTSD_COM_MODE_WRITE, comAddr, (int16*)(data), comNum);//写入protect off 命令
	if(iRet!=0)
	{
		return -1;
	}
...
int16  CFirmwareDL::ProtectOn(int16 com_type)
{
	...
	iRet = g_AbsCom->GTSD_Com_Firmware_handler(com_type, GTSD_COM_MODE_WRITE, comAddr, (int16*)(data), comNum);//写入protect on 命令
	if(iRet!=0)
	{
		return -1;
	}
...
int16  CFirmwareDL::EraseData(int16 com_type)
{
	...
	iRet				= g_AbsCom->GTSD_Com_Firmware_handler(com_type, GTSD_COM_MODE_WRITE, comAddr, (int16*)(data), comNum); //写入erase 命令
	if(iRet!=0)
	{
		return -1;
	}
...
int16 CFirmwareDL::WriteFPGAFileToFlash(int16 com_type, string pFileName, void(*tpfUpdataProgressPt)(void*,int16*),void* ptrv,int16& progress)
{
    void* ptr = ptrv;
	int iRet;
	iRet = ProtectOff(com_type);//关闭写保护
	if (iRet != 0)
	{
		return 1;
	}

	iRet = EraseData(com_type); //清除fpga flash
	if (iRet != 0)
	{
		ProtectOn(com_type);
		return 2;
	...
	iRet = ProtectOn(com_type);//open写保护
	if (iRet != 0)
	{
		return 6;
	}
